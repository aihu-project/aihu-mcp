...
|---|---|
| `src/cookbook-index.json` | the `aihu_example` MCP tool (`@aihu/mcp`) |
...
```bash
bun scripts/build-cookbook-index.ts
```

## Frontmatter contract

Every recipe MUST open with a `<!-- @cookbook -->` block (schema and construct
registry: `scripts/cookbook-lib.ts`). The build fails loudly — listing
